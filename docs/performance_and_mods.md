...
Current server-side memory attribution lane: `tools/bc test scenario mod_ram_partition --bootstrap-mode once`. It is the supported unattended path for disposable dedicated-server RAM triage against the current mod pool. The default `bisect` seed strategy clones one prepared runtime per branch and removes dependency-closed halves. For direct dependency-island A/B work, use `tools/bc test scenario mod_ram_partition --bootstrap-mode once --seed-strategy smallest_islands`, which queues the smallest removable dependency islands first and measures each island against the current baseline before any recursive narrowing. The lane records `/proc` RSS/HWM plus `jcmd` heap/native-memory summaries and persists resumable queue/results state under `~/.cache/bc/bc-mod-ram-partition` by default. Treat grouped deltas as branch-attribution evidence, not perfect isolated per-jar truth, especially when forced dependency closure removes additional jars beyond the requested seed set.
