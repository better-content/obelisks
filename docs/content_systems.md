...
Still-Beating Hearts use the highest empty main-inventory slot, then empty ender storage, and otherwise remain pending; delivery never shifts existing inventory. PlayerRevive client state is cleared when the local player changes or the target is gone, dead, in another level, or no longer bleeding, preventing a stale revive-progress overlay after death.
