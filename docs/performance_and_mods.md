...
The 2026-07-16 post-integration reset-runtime smoke passed with zero soft findings. A same-day `vs_ships_release` retry reached a playable dedicated server, then stopped in its first lifecycle lane because Moonlight's known VS forced-crash-prevention message matched `dependency_mixin_failure`; 109 physics-queue warnings were also retained. This is current automated compatibility debt, while the accepted in-game test remains the playable-stability authority.
