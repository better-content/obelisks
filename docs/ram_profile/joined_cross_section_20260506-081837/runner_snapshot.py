...
PRISM_INSTANCE = os.environ.get("PRISM_INSTANCE", "Bound to Matter-Playtest 3 - v1")
...
OUT_BASE = Path(os.environ.get("OUT_BASE", str(ROOT / "docs" / "ram_profile")))
STAMP = os.environ.get("STAMP") or datetime.now().strftime("%Y%m%d-%H%M%S")
OUT_DIR = OUT_BASE / f"joined_cross_section_{STAMP}"
SERVER_PORT = int(os.environ.get("SERVER_PORT", "25566"))
