...
function bcChemUseIngredient(input) {
    if (typeof input !== 'string') return input
    if (input.charAt(0) === '#') return { tag: input.substring(1) }
    return { item: input }
}

function bcChemUseIngredientExists(input) {
    if (!input) return false
    if (typeof input !== 'string') return true
    if (input.charAt(0) === '#') return true
    return bcChemUseExists(input)
}

function bcChemUseCanCraft(output, inputs) {
    if (!bcChemUseExists(output)) return false
    for (var i = 0; i < inputs.length; i++) {
        if (!bcChemUseIngredientExists(inputs[i])) return false
    }
    return true
}

function bcChemUseResult(output, count, chance) {
...
function bcChemUseMix(event, id, output, count, inputs, fluid, amount, heat, time) {
    if (!bcChemUseCanCraft(output, inputs)) return
    var ingredients = inputs.map(bcChemUseIngredient)
    if (fluid) ingredients.push({ fluid: fluid, amount: amount || 250 })
    var recipe = {
        type: 'create:mixing',
        ingredients: ingredients,
        results: [bcChemUseResult(output, count || 1)],
...
function bcChemUseCompact(event, id, output, count, inputs, heat) {
    if (!bcChemUseCanCraft(output, inputs)) return
    var recipe = {
        type: 'create:compacting',
        ingredients: inputs.map(bcChemUseIngredient),
        results: [bcChemUseResult(output, count || 1)],
        processingTime: 180
    }
    if (heat) recipe.heatRequirement = heat
...
function bcChemUsePressure(event, id, output, count, pressure, inputs) {
    if (!bcChemUseCanCraft(output, inputs)) return
    event.custom({
        type: 'pneumaticcraft:pressure_chamber',
        inputs: inputs.map(function (input) {
            var stack = bcChemUseIngredient(input)
...
function bcChemUseAlchemy(event, id, output, count, inputs, syphon, ticks, upgradeLevel) {
    if (!bcChemUseCanCraft(output, inputs)) return
    event.custom({
        type: 'bloodmagic:alchemytable',
        input: inputs.map(bcChemUseIngredient),
        output: bcChemUseResult(output, count || 1),
        syphon: syphon || 1000,
        ticks: ticks || 200,
...
function bcChemUseArsImbue(event, id, input, output, count, source, pedestalItems) {
    if (!bcChemUseCanCraft(output, [input])) return
    event.custom({
        type: 'ars_nouveau:imbuement',
        input: bcChemUseIngredient(input),
        output: output,
        count: count || 1,
        source: source || 500,
        pedestalItems: pedestalItems.map(function (item) {
            return { item: bcChemUseIngredient(item) }
        })
...
ServerEvents.recipes(function (event) {
    // Electronics: boards, wafers, processors-in-waiting.
     ...
     bcChemUseMix(event, 'powergrid/unetched_circuit_ceramic_board', 'powergrid:unetched_circuit', 2, [
        '#forge:plates/copper',
        'chemlib:aluminum_oxide',
        'chemlib:silicon_dioxide',
        'morered:red_alloy_wire'
    ], null, 0, null, 160)
     bcChemUseMix(event, 'powergrid/incomplete_circuit_copper_chloride', 'powergrid:incomplete_circuit', 2, [
        'powergrid:unetched_circuit',
        'chemlib:copper_chloride',
        'chemlib:phosphorus'
    ], 'chemlib:hydrochloric_acid_fluid', 250, 'heated', 220)
     bcChemUsePressure(event, 'pneumaticcraft/empty_pcb_ceramic_laminate', 'pneumaticcraft:empty_pcb', 2, 2.0, [
        '#forge:plates/copper',
        'chemlib:aluminum_oxide',
     ...
     bcChemUsePressure(event, 'pneumaticcraft/transistor_doped_silicon', 'pneumaticcraft:transistor', 2, 2.5, [
        'chemlib:silicon',
        'chemlib:phosphorus',
        '#forge:nuggets/gold'
    ])
     bcChemUsePressure(event, 'pneumaticcraft/capacitor_copper_sulfate', 'pneumaticcraft:capacitor', 2, 2.0, [
        '#forge:plates/copper',
        'chemlib:copper_ii_sulfate',
        'chemlib:aluminum_oxide'
    ])
     bcChemUsePressure(event, 'oc2r/silicon_wafer_pure_silica', 'oc2r:silicon_wafer', 4, 2.5, [
        'chemlib:silicon_dioxide',
        'chemlib:phosphorus',
        'chemlib:aluminum_oxide'
    ])
     bcChemUsePressure(event, 'oc2r/transistor_phosphorus_doped', 'oc2r:transistor', 2, 2.5, [
        'oc2r:silicon_wafer',
        'chemlib:phosphorus',
        '#forge:nuggets/gold'
    ])
     bcChemUseMix(event, 'ae2/printed_silicon_chemical', 'ae2:printed_silicon', 2, [
        'chemlib:silicon',
        'chemlib:silicon_dioxide',
        'chemlib:phosphorus'
    ], null, 0, null, 160)
     bcChemUseMix(event, 'ae2/printed_logic_gold_etch', 'ae2:printed_logic_processor', 1, [
        '#forge:ingots/gold',
        'ae2:printed_silicon',
        'chemlib:copper_chloride'
    ], null, 0, null, 160)
     bcChemUseMix(event, 'ae2/printed_calculation_certus_etch', 'ae2:printed_calculation_processor', 1, [
        'ae2:certus_quartz_crystal',
        'ae2:printed_silicon',
        'chemlib:silicon_dioxide'
    ], null, 0, null, 160)
     bcChemUseMix(event, 'ae2/printed_engineering_diamond_etch', 'ae2:printed_engineering_processor', 1, [
        'minecraft:diamond',
        'ae2:printed_silicon',
        'chemlib:aluminum_oxide'
    ], null, 0, null, 160)
    ...
    // Magic reagent alternatives.
     bcChemUseAlchemy(event, 'bloodmagic/blank_slate_lime_batch', 'bloodmagic:blankslate', 2, [
        '#forge:stone',
        'chemlib:calcium_carbonate',
        'chemlib:iron'
    ], 1200, 180, 0)
     bcChemUseAlchemy(event, 'bloodmagic/reinforced_slate_iron_oxide_batch', 'bloodmagic:reinforcedslate', 2, [
        'bloodmagic:blankslate',
        'chemlib:iron_oxide',
        'chemlib:sulfur'
    ], 2600, 240, 1)
     bcChemUseAlchemy(event, 'bloodmagic/imbued_slate_phosphate_batch', 'bloodmagic:infusedslate', 2, [
    ...
    for (var r = 0; r < runes.length; r++) {
     ...
     bcChemUseAlchemy(event, 'occultism/chalk_gold_phosphate', 'occultism:chalk_gold', 2, [
        'occultism:chalk_white',
        'chemlib:phosphate',
        '#forge:nuggets/gold',
        'bloodmagic:reinforcedslate'
    ], 2800, 220, 1)
     bcChemUseAlchemy(event, 'hexerei/blood_bottle_stabilized_salt', 'hexerei:blood_bottle', 2, [
        'minecraft:glass_bottle',
        'chemlib:sodium_chloride',
        'chemlib:phosphorus',
        'minecraft:redstone'
    ], 2200, 220, 1)
     bcChemUseArsImbue(event, 'ars/source_gem_beryl_silica', 'minecraft:amethyst_shard', 'ars_nouveau:source_gem', 2, 900, [
        'chemlib:beryl',
        'chemlib:silicon_dioxide',
        'bloodmagic:reinforcedslate'
    ])
     bcChemUseArsImbue(event, 'ars/source_gem_phosphorus_charge', 'ars_nouveau:source_gem', 'ars_nouveau:source_gem_block', 1, 1800, [
        'chemlib:phosphorus',
        'chemlib:aluminum_oxide',
        'bloodmagic:infusedslate'
    ])
    ...
    // Growable/body economy sinks.
     bcChemUseMix(event, 'minecraft/bone_meal_phosphate_bulk', 'minecraft:bone_meal', 8, [
        'chemlib:phosphate',
        'chemlib:calcium',
        'chemlib:phosphorus'
     ...
     bcChemUseMix(event, 'farmersdelight/rich_soil_npk', 'farmersdelight:rich_soil', 2, [
        'farmersdelight:organic_compost',
        'chemlib:phosphoric_acid',
        'chemlib:calcium_carbonate'
    ], 'minecraft:water', 250, null, 240)
     bcChemUseMix(event, 'minecraft/wheat_protein_feed', 'minecraft:wheat', 4, [
     ...
     bcChemUseMix(event, 'minecraft/string_collagen_fiber', 'minecraft:string', 4, [
        'minecraft:bone_meal',
        'chemlib:calcium',
        'chemlib:sulfur'
    ], 'minecraft:water', 250, null, 160)
     bcChemUseMix(event, 'minecraft/slime_ball_polymerized_biomass', 'minecraft:slime_ball', 2, [
        'minecraft:sugar',
        'chemlib:carbon',
        'chemlib:sulfur'
    ], 'chemlib:acetic_acid_fluid', 250, null, 220)
