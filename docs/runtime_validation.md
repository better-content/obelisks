...
The headful lane remains failed and blocks a playable-stability claim. Source inspection pinned Eureka's menu geometry, Assemble packet path, five-block BFS expectation, and VS teleport syntax in `tools/vs_ships_client_contract.properties`. The Xvfb client now completes that exact interaction and the server reports five assembled blocks plus one registered ship, but AI screenshot inspection found the post-assembly ship absent. The deterministic follow-up `/tmp/btm-vs-core-dh-off-scored` reproduced this with DH removed: fixture render score `0.08176`, assembled render score `0.0`, classifier `assembled_render_failure`. The current DH-enabled run showed the same visual result, so this is not a DH-only defect. Mount, movement, reconnect, save/reload, two-client sync, Clockwork, Trackwork, and broad pack tests remain gated behind the core render failure.
