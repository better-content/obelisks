...
The late lava-diving band is Overworld terrain below Y 0, enabled by Tectonic `min_y = -64` and lava tunnels. This lower band is an authored progression zone rather than ordinary branch mining: Born in Chaos monsters only spawn below Y 0, and its ores are meant to be recovered as prepared expeditions with real lava-survival gear. Uranium and thorium are geographically exclusive to signalled ADLODS fields whose ore bodies replace lava in this band, making nuclear fuel a scarce surveyed objective while preserving lava diving as the extraction challenge. `datapacks/realistic_ores_lava_depths` separately retains only the exposed osmiridium lava-sulfide scatter; the custom `realisticores:lava_exposed_ore` feature places those blocks only where they touch lava. Magma cubes are the current lava-band hazard, and Protection Pixel Tosaki gear is the intended post-AE2 diving suit for this route.
