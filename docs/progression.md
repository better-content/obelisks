...
Pillager campaign pressure is adaptive by warband rather than global time scaling. New warbands open at the gentlest strength tier, each successfully defeated raid increases that warband's future raid strength by 1, and each raid that kills a player reduces its future strength by 1 while ending that active push for roughly a day and marking the death site with the warband's banner. This keeps successful settlements under escalating surface pressure while giving losing players an automatic easing path instead of immediate repeat pressure. Campaign squads materialize only on deterministic dry-land sites in loaded chunks near the intended approach point; coastal failures stay retryable instead of falling back to boats. Warbands persist a fixed archetype so rally warlords and campaign squads keep the same visual/mechanical identity, with warlords tuned as large armored boss targets. Rally drift remains an off-screen pressure mechanic only: warbands do not relocate their rally while the current rally chunk or the proposed destination chunk is already loaded.
