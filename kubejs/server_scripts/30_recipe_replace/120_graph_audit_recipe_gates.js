...
function bcGraphItemExists(id) {
    try { return Item.exists(id) } catch (e) { return false }
}

function bcGraphRemoveOutputs(event, outputs) {
    for (var i = 0; i < outputs.length; i++) event.remove({ output: outputs[i] })
}

function bcGraphShaped(event, output, pattern, keys, id) {
    if (!bcGraphItemExists(output)) return
...
ServerEvents.recipes(function (event) {
    ...
    // Early Create utility should still depend on the andesite machine casing tier once it is block-machine-like.
     bcGraphShaped(event, 'create:fluid_pipe', [
        ' C ',
        'SAS',
        ' C '
    ], {
        ...
        A: BC_GRAPH_GATE.andesite
    }, 'kubejs:graph_gate/create/fluid_pipe')

     bcGraphShaped(event, 'create:mechanical_pump', [
        ' S ',
        'FAF',
        ' C '
    ], {
     ...
     bcGraphShaped(event, 'create:track_observer', [
        ' R ',
        'CAC',
        ' T '
    ], {
    ...
    // Create Connected and diesel logistics are brass-era infrastructure.
     bcGraphShaped(event, 'create_connected:sequenced_pulse_generator', [
        'RCR',
        'ABA',
        'RCR'
    ], {
        ...
        B: BC_GRAPH_GATE.brass
    }, 'kubejs:graph_gate/create_connected/sequenced_pulse_generator')

     bcGraphShaped(event, 'create_connected:linked_transmitter', [
        ' E ',
        'RBR',
        ' P '
    ], {
    ...
    // Power Grid / New Age heat infrastructure should not be simple Create-era crafting.
     bcGraphShaped(event, 'heatsync:heat_pipe', [
        'SCS',
        'PBP',
        'SCS'
    ], {
        ...
        P: 'create:fluid_pipe',
        B: BC_GRAPH_GATE.power
    }, 'kubejs:graph_gate/heatsync/heat_pipe')

     bcGraphShaped(event, 'heatsync:coolant_exchanger', [
        'PHP',
        'CBC',
        'PHP'
    ], {
    ...
    // Chunk loading is remote-site infrastructure and must sit behind power/compute logistics.
     bcGraphShaped(event, 'create_power_loader:empty_andesite_chunk_loader', [
        'SPS',
        'CBC',
        'SPS'
    ], {
     ...
     bcGraphShaped(event, 'create_power_loader:empty_brass_chunk_loader', [
        'SPS',
        'CBC',
        'SPS'
    ], {
    ...
    // AE2 addon oversize interface is local-intelligence scale, not a cheap alternate interface.
     bcGraphShaped(event, 'expatternprovider:oversize_interface', [
        'IPI',
        'BAB',
        'IPI'
    ], {
