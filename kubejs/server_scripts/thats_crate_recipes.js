...
// Packs with Create compacting (all tiers) + unpacks with Create milling (all tiers).
    ...
    for (var bi = 0; bi < BASES.length; bi++) {
        ...
        for (var tier = 1; tier <= MAX_TIER; tier++) {
            ...
            event.custom({
                ...
                results: [{ item: outId }]
            }).id('kubejs:crates/pack/' + idPath(base) + '/t' + tier);

            addedPack++;

            // UNPACK: 1x out -> 9x in (Create Milling / Millstone)
            event.custom({
                type: "create:milling",
