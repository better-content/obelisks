...
tools/bc test scenario-headful rain_collector_visuals --bootstrap-mode once
tools/bc test scenario-headful revival --bootstrap-mode once
...
- `rain_collector_visuals` is a functional no-shader fixture. The server authors the platform, rain, camera, and collector states; the client only renders while its framebuffer is captured, with no menu or interaction input automation. The lane disables DH rendering/generation and requires vision review of all five states, textures, transparency, clipping, and z-fighting.
...
`tools/bc test smoke` bootstraps a fresh server with the deterministic `better-content-validation-v1` seed, prunes stale runtime mods, boots the server, scans hard log failures, runs the Unearthed replacement regression guard, and runs the strict runtime suite. Authenticated server bundles do not ship that validation seed. A startup timeout captures a JVM thread dump, heap summary, native-memory summary, and `/proc` status before shutdown. The guard reads full Overworld Anvil chunks directly and treats Y 64 as the band boundary: below it, vanilla stone-family residue must remain under the bounded geology ratio; at and above it, Unearthed rock replacement and regolith/overgrown surface evidence must both be present while vanilla rock and surface-host ratios remain bounded. It writes `unearthed-replacement-audit.json` into the run evidence directory. `worldgen_sampling` reruns the guard after each Overworld sampling cycle, and the same guard can be run independently with `tools/bc test unearthed-replacement --instance /path/to/fresh/runtime`.
...
Playtest stabilization evidence from 2026-07-19: `~/.cache/bc/content-smoke` passed fresh reset-runtime smoke with 0 soft findings and 0 tick-behind warnings. The same source passed `opening_progression --cycles 1 --bootstrap-mode once` and the Pillager Campaigns `nemesis_cycle` lane. The passive client/server `rain_collector_visuals` lane passed technical capture and GPT-5 vision review: empty through full levels were readable under rain with no missing texture, clipping, transparency, or z-fighting defect.

Revival replacement evidence from 2026-07-19: the standalone `revival` source passed its JUnit state/rules suite, reobfuscated assembly, and headless Forge GameTest. The pack passed Kotlin tooling tests, full static validation, server/client sync dry-runs and applies, and fresh reset-runtime smoke at `~/.cache/bc/revival-smoke` with 0 soft findings. `tools/bc test scenario-headful revival --bootstrap-mode once --run-root ~/.cache/bc/revival-headful` also passed the replacement/load contract: the staged runtime contained `revival`, contained no PlayerRevive artifact, synced the authored common config, and completed strict runtime smoke. This lane is a load and replacement gate; it does not yet automate a two-player revive interaction or visual judgment of the downed HUD.
...
`local` is the single-cycle local confidence lane and uses an 81-chunk Overworld census: smaller 9- and 25-chunk windows legitimately produced no sparse ADLOD surface indicators on the deterministic validation seed. `quick` is the short seeded Overworld lane, and `release` broadens the dimension set and sample count. All profiles are validated against `tools/worldgen_sampling_contract.json` before the runtime backend starts.
...
Ore-generation regression coverage now runs `bc_ore_audit` against virgin far chunks and fails on a world floor other than Y -128, low Realistic Ores density or diversity, any surviving vanilla Overworld ore, or missing ADLODS surface samples. The 2026-07-16 quick census on the corrected runtime measured 6,375 Realistic Ores blocks across 25 ore IDs, 83 crushed-rock surface samples, and zero vanilla ore blocks in 25 chunks. A separate manual fresh-region census measured 8,083 blocks across 26 IDs and 35 samples with zero vanilla ores. The full quick scenario reached and passed its ore gate, then failed the separate whole-world Unearthed replacement guard because the reused runtime retained older chunks; this is not clean full-scenario evidence and should be refreshed on a disposable runtime after the Unearthed aboveground heuristic is stabilized.
...
The retained 2026-07-10 evidence covers VS/Eureka only and predates reactivation of Clockwork and Trackwork. It does not establish full-family stability by itself and remains historical context rather than the current acceptance decision.
...
`~/.cache/bc/bc-vs-release-all` completed the full nine-lane release orchestrator on 2026-07-11. Server lifecycle passed across Overworld, Lost Cities, and Earth orbit, and all ten server isolation variants passed. No crash report, JVM fatal error, dependency/mixin failure, or save corruption was produced. All seven historical release client lanes failed cleanly: `mount_camera_failure` for core and both C2ME-disabled combined fixtures, `menu_packet_failure` for Clockwork, `onboarding_failure` for Trackwork, and network-timeout `client_disconnect_failure` for current and DH-disabled combined fixtures. Disabling DH did not remove the timeout; disabling C2ME allowed combined assembly/translation but did not remove the mount failure. Retained logs also contain Dimension Drink far-chunk writes, ModernFix wrong-thread reload-listener warnings, and Moonlight's VS forced-crash prevention message.

`~/.cache/bc/bc-vs-release-simplified` reran all nine release lanes on 2026-07-12 after removing automated mount/camera, movement, and observer tests. Server lifecycle and all ten isolation variants passed. Clockwork plus combined C2ME-disabled and DH+C2ME-disabled clients passed all server-authoritative assembly, transform, reconnect, and restart assertions, with only `render_environment_inconclusive` under Xvfb/llvmpipe. Core, Trackwork, combined current, and combined DH-disabled stopped at the source-fixture visibility threshold before assembly. No crash report, JVM fatal error, dependency/mixin failure, or player-control failure occurred. The remaining automated limitation is the software-renderer visibility oracle; manual playtests own camera, control, driving, and observer behavior. On 2026-07-14 the source config disabled ModernFix's integrated-server watchdog for client/singleplayer runs because VS physics backlog, DH generation, and heavy worldgen experiments can trip its 40-second integrated tick report without identifying a pack crash. Physics-frame queue warnings remain diagnostic evidence, and dedicated-server validation still treats watchdogs/thread dumps as fatal.

On 2026-07-16, user-directed in-game testing accepted the VS family as playable-stable and released the prototype freeze only for its transport integration. Current authored progression is primitive TConstruct-part Eureka boats, railway/precision-gated Trackwork land vehicles, and Aether-plus-Airtight Eureka/Clockwork powered flight. This manual signoff supplies the control judgment outside the automated harness; future regressions still use the VS scenario family and fresh runtime evidence.

Post-integration validation on 2026-07-16 passed a fresh reset-runtime smoke with zero soft findings. The full `vs_ships_release` retry reached `Done` and began its fixture workload, but the first server lifecycle lane classified Moonlight's known VS forced-crash-prevention message as `dependency_mixin_failure` and retained 109 physics-queue warnings, so the remaining eight lanes did not run. Treat that result as unresolved automated compatibility evidence, not as a replacement for the accepted in-game control test.

Current fresh custom-mod and bundle evidence from 2026-07-16: all 19 bundled custom mods passed clean source `test` and `assemble` tasks and were deployed only from each ForgeGradle project's `build/reobfJar/output.jar`. `~/.cache/bc/fresh-custom-mods-smoke` passed a reset-runtime smoke and follow-up runtime validation. A separate always-bootstrap quick client smoke exposed a seed-sensitive cluster of late vanilla stone/granite around Y=48–61; the final `bcfixes` post-SGI cleanup reduced the same 529-chunk audit from 0.0579% to 0.00127% residual underground vanilla rock, while aboveground passed at 0.00320%. The rerun then passed fresh server startup, client launch/join/render checks, playable-world, save, tick, and hard-log gates with zero soft findings. Fresh CurseForge/client and complete-server archives passed ZIP integrity checks, all 19 embedded custom-jar hashes matched the tested repository artifacts, the client archive contained the exact tracked `options.txt`, and the extracted server archive passed runtime-shape inspection.
...
- Extend the passing Unearthed/Hyle replacement audit to multi-seed statistical sampling when worldgen distributions are next tuned.
