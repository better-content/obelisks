...
These lanes currently exercise pinned Valkyrien Skies `valkyrienskies-120-2.4.11.jar` and Eureka `eureka-1201-1.6.3.jar` as stability-discovery surfaces only. VS: Clockwork `clockwork-0.5.6.jar` and Trackwork `trackwork-1.20.1-1.2.4.jar` are deferred until the core assembly/lifecycle gate passes; Shoulder Surfing is removed as VS-incompatible. Raw evidence stays under `/tmp/btm-vs-*` with `summary.json`, `summary.txt`, `metrics.json` where applicable, console logs, copied latest logs, `commands.log`, registry snapshots, and an AI-reviewable screenshot manifest. A classified failure is useful evidence for follow-up patching; an unclassified failure means the classifier surface or evidence capture must be improved before claiming a known failure mode. The headful harness treats assembly and explicit ship translation as server-state assertions; screenshots are supplemental. Xvfb/llvmpipe or unsupported GLSL runs are `render_environment_inconclusive`, not confirmed VS rendering failures.

`vs_ships_matrix` mutates only copied disposable runtimes under `/tmp`. Current VS/Eureka DH and C2ME variants remain available; the VS+Clockwork, VS+Trackwork, and full-family variants are retained for later use after those add-ons return. Do not make these lanes pass by disabling the VS feature family in source.
...
The current headful evidence does not establish a VS rendering bug. Source inspection pinned Eureka's menu geometry and packet path plus VS assembly, seamless chunk, Sodium renderer, and teleport behavior in `tools/vs_ships_client_contract.properties` and the harness. The fixture contains four unique blocks; Eureka nondeterministically logs `5..8` BFS entries because its initial neighbor stack can process connected positions twice. DH-enabled and DH-off runs assemble one ship at the source transform, and the server confirms an explicit ship translation; the old screenshot assertion was invalid because assembly preserves the source transform. The container client uses Xvfb/llvmpipe and logs an unsupported GLSL capability, so screenshot-based rendering is environment-inconclusive. A hardware-rendered client remains required before claiming playable visual stability. Mount, movement, save/reload, two-client sync, Clockwork, Trackwork, and broad pack tests remain gated until the focused transform/lifecycle lane is trustworthy.
